    ...
    if (newExpr === 1) {
      ...
      for (let i = 0; i < 20; i++) {
        dripsRef.current.push({
          ...
          opacity: 1,
          color: Math.random() > 0.5 ? '#22c55e' : '#fbbf24',
          life: 0,
        });
      }
      setTimeout(() => {
    ...
    const animate = () => {
      ...
      if (isMoving && velocity > 2) {
        posHistoryRef.current.forEach((pos, i) => {
          ...
          dripCtx.fillStyle = `rgba(34, 197, 94, ${opacity})`;
          dripCtx.fill();
          dripCtx.beginPath();
          dripCtx.arc(pos.x + ghostSize / 2, pos.y + ghostSize / 2, size * 0.5, 0, Math.PI * 2);
          dripCtx.fillStyle = `rgba(251, 191, 36, ${opacity * 0.6})`;
          dripCtx.fill();
        });
      }
      ...
      miniGhostsRef.current = miniGhostsRef.current.filter((mg) => {
        ...
        dripCtx.fillStyle = '#fbbf24';
        dripCtx.fill();
        dripCtx.fillStyle = '#22c55e';
        dripCtx.beginPath();
  ...
  return (
    <>
      ...
      >
        {speechBubble && (
          <div
            className="absolute -top-14 left-1/2 -translate-x-1/2 px-4 py-2 rounded-xl whitespace-nowrap font-caveat text-lg pointer-events-none"
            style={{
              background: 'rgba(15, 31, 15, 0.9)',
              border: '1px solid rgba(34, 197, 94, 0.3)',
              color: expression === 2 ? '#ef4444' : '#22c55e',
        ...
        <div
          ...
          style={{
            ...
            height: ghostSize,
            filter: isHovered
              ? 'drop-shadow(0 0 25px #22c55e) drop-shadow(0 0 50px #fbbf24) brightness(1.2)'
              : undefined,
